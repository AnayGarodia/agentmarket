...
def load_builtin_specs_part4() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "description": "Use when you need to measure the real latency and status profile of an HTTP endpoint. Sends multiple live requests with bounded concurrency, returns p50/p95/p99 latency, status-code distribution, histogram buckets, and sample failures. VARIABLE BILLING: tiered by `requests`, with the standard 10% platform fee added on top. Approximate caller charges (after fee): ~$0.03 for 1 request, ~$0.06 for 5, ~$0.10 for 10, ~$0.53 for 50, ~$0.99 for 100. Pass requests=5 for the cheapest meaningful sample. The exact charge is always returned in `pricing_units.caller_charge_cents`.",
