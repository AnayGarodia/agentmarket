"""Built-in agent specs — batch 3 (regex_tester, cron_expression_parser,
ssl_certificate_decoder, diff_analyzer, k8s_manifest_validator)."""
...
def load_builtin_specs_part8() -> list[dict]:
    ...
    return [
        {
            "agent_id": REGEX_TESTER_AGENT_ID,
            "name": "Regex Tester",
            "description": (
                "Execute Python regular expressions against one or more test strings "
                "and return structured match results including full match text, span "
                "positions, positional groups, and named captures. Supports IGNORECASE, "
                "MULTILINE, DOTALL, VERBOSE, and ASCII flags. All matches are returned "
                "(findall semantics). Invalid patterns are reported per-result without "
                "aborting the batch."
            ),
            "endpoint_url": "internal://regex_tester",
            "price_per_call_usd": 0.002,
            "tags": ["regex", "pattern-matching", "testing", "developer-tools"],
            "is_featured": True,
            "match_keywords": [
                "regex",
                "regular expression",
            ...
            "block_keywords": ["generate regex", "write a regex", "explain regex"],
            "input_schema": {
                "type": "object",
                "properties": {
                    "pattern": {
                        "type": "string",
                        "description": "Single regex pattern to test.",
                    },
                    "test_string": {
                        "type": "string",
                        "description": "Single string to match against.",
                    },
                    "patterns": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Batch of patterns (max 10).",
                    },
                    "strings": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Batch of strings to test each pattern against (max 20).",
                    },
                    "flags": {
                        "type": "array",
                        "items": {
                        ...
                        "description": "Optional regex flags to apply.",
                    },
                },
            },
            "output_schema": {
                "type": "object",
                "properties": {
                    "patterns_tested": {"type": "integer"},
                    "strings_tested": {"type": "integer"},
                            ...
                            "properties": {
                                ...
                                "matches": {
                                    ...
                                    "items": {
                                        "type": "object",
                                        "properties": {
                                            "full_match": {"type": "string"},
                                            "start": {"type": "integer"},
                                            "end": {"type": "integer"},
                                            "groups": {"type": "array"},
                                            "named_groups": {"type": "object"},
                                        },
                                    },
                                },
                ...
                "required": ["patterns_tested", "strings_tested", "total_matches", "results"],
            },
            "output_examples": [
                {
                    "input": {
                        "pattern": r"(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})",
                        "test_string": "Release date: 2026-05-09, patch 2026-06-01",
                    },
                    "output": {
                        "patterns_tested": 1,
                        "strings_tested": 1,
                        ...
                        "results": [
                            {
                                "pattern": r"(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})",
                                "string": "Release date: 2026-05-09, patch 2026-06-01",
                                "match_count": 2,
                                "matches": [
                                    {
                                        "full_match": "2026-05-09",
                                        "start": 14,
                                        "end": 24,
                                        "groups": ["2026", "05", "09"],
                                        "named_groups": {"year": "2026", "month": "05", "day": "09"},
                                    },
                                    {
                                        "full_match": "2026-06-01",
                                        "start": 33,
                                        "end": 43,
                                        "groups": ["2026", "06", "01"],
                                        "named_groups": {"year": "2026", "month": "06", "day": "01"},
                                    },
                                ],
                                "error": None,
                            }
        ...
        {
            "agent_id": CRON_EXPRESSION_PARSER_AGENT_ID,
            "name": "Cron Expression Parser",
            "description": (
                "Parse 5-field and 6-field cron expressions (plus @hourly, @daily, @weekly, "
                "@monthly, @yearly macros), compute the next N scheduled run times in any "
                "IANA timezone, and detect pathological schedules such as expressions that "
                "never fire or that trigger more than 100 times per day. Uses croniter when "
                "available for full edge-case coverage (L, W, #, ranges), falls back to "
                "a built-in walker for standard expressions."
            ),
            "endpoint_url": "internal://cron_expression_parser",
            "price_per_call_usd": 0.002,
            "tags": ["cron", "scheduling", "time", "developer-tools"],
            "is_featured": True,
            "match_keywords": [
                "cron",
                "cron expression",
            ...
            "input_schema": {
                ...
                "properties": {
                    "expression": {
                        "type": "string",
                        "description": "5-field cron (min hr dom mon dow), 6-field with seconds, or @macro.",
                    },
                    "n": {
                        "type": "integer",
                    ...
                    "reference_time": {
                        "type": "string",
                        "description": "ISO-8601 datetime to compute from (default: now).",
                    },
                },
                "required": ["expression"],
            },
            "output_schema": {
                ...
                "properties": {
                    ...
                    "valid": {"type": "boolean"},
                    "field_breakdown": {"type": "object"},
                    "next_runs": {"type": "array", "items": {"type": "string"}},
                    "frequency": {"type": "object"},
                    "warnings": {"type": "array", "items": {"type": "string"}},
                    "timezone": {"type": "string"},
                    "tool_used": {"type": "string", "enum": ["croniter", "builtin"]},
                },
                "required": ["expression", "valid", "next_runs", "timezone"],
            },
            "output_examples": [
                {
                    "input": {"expression": "0 2 * * 1", "n": 3, "timezone": "America/New_York"},
                    "output": {
                        "expression": "0 2 * * 1",
                        "description": "Every Monday at 02:00",
                        "valid": True,
                        "field_breakdown": {
                            "minute": "0",
                            "hour": "2",
                        ...
                        "next_runs": [
                            "2026-05-11T02:00:00-04:00",
                            "2026-05-18T02:00:00-04:00",
                            "2026-05-25T02:00:00-04:00",
                        ],
                        "frequency": {"runs_per_day": 0.14, "runs_per_hour": 0.006, "interval_minutes": None},
                        "warnings": [],
