...
def evaluate_failed_call_spike(jobs_metrics: dict[str, Any], thresholds: Thresholds) -> list[Alert]:
    ...
    if pct >= thresholds.failed_call_pct_critical:
        return [Alert("failed_call_spike", "critical", f"{failed}/{total} jobs failed today ({pct:.1%})", detail)]
    if pct >= thresholds.failed_call_pct_warn:
        return [Alert("failed_call_spike", "warn", f"{failed}/{total} jobs failed today ({pct:.1%})", detail)]
    return []


def evaluate_refund_spike(payments: dict[str, Any], thresholds: Thresholds) -> list[Alert]:
    ...
    detail = {"refunds_cents": refunds, "charges_cents": charges, "ratio": round(ratio, 4)}
    if ratio >= thresholds.refund_ratio_critical:
        return [Alert("refund_spike", "critical", f"refund ratio {ratio:.1%} (refunds {refunds}c / charges {charges}c)", detail)]
    if ratio >= thresholds.refund_ratio_warn:
        return [Alert("refund_spike", "warn", f"refund ratio {ratio:.1%} (refunds {refunds}c / charges {charges}c)", detail)]
    return []


def evaluate_ledger_drift(reconcile: dict[str, Any], thresholds: Thresholds) -> list[Alert]:
    ...
    if drift >= thresholds.ledger_drift_cents_critical or mismatches > 0:
        return [Alert("ledger_drift", "critical", f"reconciliation drift {drift}c, mismatches {mismatches}", detail)]
    if drift >= thresholds.ledger_drift_cents_warn:
        return [Alert("ledger_drift", "warn", f"reconciliation drift {drift}c", detail)]
    return []


def evaluate_degraded_agents(agents: Iterable[dict[str, Any]], thresholds: Thresholds) -> list[Alert]:
    out: list[Alert] = []
    for agent in agents or []:
        ...
        if success_f <= thresholds.agent_success_critical:
            out.append(Alert("degraded_agent", "critical", f"{name} success {success_f:.1%} over {total_calls} calls", detail))
        elif success_f <= thresholds.agent_success_warn:
            out.append(Alert("degraded_agent", "warn", f"{name} success {success_f:.1%} over {total_calls} calls", detail))
    return out
...
def evaluate_search_empty(probe_results: dict[str, Any]) -> list[Alert]:
    ...
    for query, count in (probe_results or {}).items():
        if int(count) <= 0:
            out.append(Alert("search_empty", "warn", f"search returned 0 results for {query!r}", {"query": query}))
    return out


def evaluate_worker_backlog(jobs_metrics: dict[str, Any], thresholds: Thresholds) -> list[Alert]:
    pending = int(jobs_metrics.get("pending") or 0)
    detail = {"pending": pending}
    if pending >= thresholds.worker_backlog_critical:
        return [Alert("worker_backlog", "critical", f"{pending} pending jobs in queue", detail)]
    if pending >= thresholds.worker_backlog_warn:
        return [Alert("worker_backlog", "warn", f"{pending} pending jobs in queue", detail)]
    return []


# ─── HTTP wiring ─────────────────────────────────────────────────────────────

_PROBE_QUERIES = ["lint python code", "type check python", "scan secrets", "audit dependencies"]


def _get(base: str, path: str, headers: dict[str, str], timeout: float) -> dict[str, Any]:
    resp = requests.get(f"{base}{path}", headers=headers, timeout=timeout)
    resp.raise_for_status()
    return resp.json() if resp.content else {}


def _post(base: str, path: str, headers: dict[str, str], body: Any, timeout: float) -> dict[str, Any]:
...
def _collect_metrics(base: str, api_key: str, timeout: float) -> dict[str, Any]:
    ...
    for q in _PROBE_QUERIES:
        try:
            payload = _post(base, "/registry/search", headers, {"query": q, "limit": 5}, timeout)
            probe[q] = len(payload.get("results") or [])
        except Exception:
            probe[q] = 0
    metrics["search_probes"] = probe
    metrics["agents"] = (_get(base, "/registry/agents", headers, timeout) or {}).get("agents") or []
    return metrics


def evaluate_all(metrics: dict[str, Any], thresholds: Thresholds | None = None) -> list[Alert]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not api_key:
        print(json.dumps({"error": "AZTEA_API_KEY required for /ops endpoints"}), file=sys.stderr)
        return 2
    try:
        metrics = _collect_metrics(base, api_key, timeout)
    except Exception as exc:
        print(json.dumps({"error": f"metrics collection failed: {exc}"}), file=sys.stderr)
        return 2
