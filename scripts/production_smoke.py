...
def _request(method: str, path: str, *, api_key: str | None = None, body: Any = None) -> dict[str, Any]:
...
def _auth() -> str:
    ...
    if not email or not password:
        raise SmokeFailure("Set AZTEA_API_KEY or AZTEA_SMOKE_EMAIL/AZTEA_SMOKE_PASSWORD.")
    payload = _request("POST", "/auth/login", body={"email": email, "password": password})
...
def _search(api_key: str, query: str, *, limit: int = 5) -> list[dict[str, Any]]:
    payload = _request("POST", "/registry/search", api_key=api_key, body={"query": query, "limit": limit})
...
def _find_agent(api_key: str, query: str, name_contains: str) -> dict[str, Any]:
    for agent in _search(api_key, query, limit=8):
        if name_contains.lower() in str(agent.get("name") or "").lower():
            return agent
    raise SmokeFailure(f"Could not find agent containing {name_contains!r} for query {query!r}.")
...
def _poll_job(api_key: str, job_id: str, *, timeout_s: float = 45.0) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if str(last.get("status") or "").lower() in {"complete", "failed"}:
            return last
        time.sleep(1.0)
    raise SmokeFailure(f"Job {job_id} did not finish before timeout. Last status: {last}")
...
def _check_mcp_manifest(api_key: str) -> None:
    ...
    proc = subprocess.run(cmd, cwd=os.getcwd(), text=True, capture_output=True, timeout=TIMEOUT)
    ...
    if not {"aztea_search", "aztea_describe", "aztea_call"} <= tool_names:
        raise SmokeFailure(f"MCP lazy tools missing from manifest: {sorted(tool_names)}")
...
def main() -> int:
    ...
    try:
        try:
            ...
            linter = run("search linter", lambda: _find_agent(api_key, "lint python code", "Linter"))
            type_checker = run("search type checker", lambda: _find_agent(api_key, "type check python", "Type Checker"))
            linter_id = linter["agent_id"]
            type_id = type_checker["agent_id"]

            run("estimate", lambda: _request("POST", f"/agents/{linter_id}/estimate", api_key=api_key, body={"code": "x=1\n"}))
            run("direct call", lambda: _request("POST", f"/registry/agents/{linter_id}/call", api_key=api_key, body={"code": "import os\nx=1\n", "language": "python"}))

            async_job = run("async submit", lambda: _request("POST", "/jobs", api_key=api_key, body={"agent_id": linter_id, "input_payload": {"code": "import os\nx=1\n", "language": "python"}}))
            run("async complete", lambda: _poll_job(api_key, async_job["job_id"]))

            batch = run("batch submit", lambda: _request("POST", "/jobs/batch", api_key=api_key, body={"jobs": [
                {"agent_id": linter_id, "input_payload": {"code": "x=1\n", "language": "python"}},
                {"agent_id": type_id, "input_payload": {"code": "def f(x: int) -> str:\n    return x\n", "language": "python"}},
            ]}))
            for item in batch.get("jobs") or []:
                if item.get("job_id"):
                    run(f"batch job {item['job_id']}", lambda job_id=item["job_id"]: _poll_job(api_key, job_id))

            compare = run("compare", lambda: _request("POST", "/jobs/compare", api_key=api_key, body={"agent_ids": [linter_id, type_id], "input_payload": {"code": "x=1\n", "language": "python"}}))
            compare_id = compare.get("compare_id")
            if compare_id:
                run("compare status", lambda: _request("GET", f"/jobs/compare/{compare_id}", api_key=api_key))

            run("recipe", lambda: _request("POST", "/recipes/audit-deps/run", api_key=api_key, body={"input_payload": {"manifest": "{\"dependencies\":{\"lodash\":\"4.17.20\"}}"}}))

            bad = run("refund failure submit", lambda: _request("POST", "/jobs", api_key=api_key, body={"agent_id": linter_id, "input_payload": {"code": 12345}}))
            failed = run("refund failure complete", lambda: _poll_job(api_key, bad["job_id"]))
            if failed.get("status") != "failed":
                raise SmokeFailure(f"Expected malformed job to fail, got {failed.get('status')}.")

            run("ledger reconcile", lambda: _request("GET", "/ops/payments/reconcile", api_key=api_key))
        except SmokeFailure:
            pass  # already recorded in checks; report below
    finally:
        print(json.dumps({"base_url": BASE_URL, "checks": [c.__dict__ for c in checks]}, indent=2))
